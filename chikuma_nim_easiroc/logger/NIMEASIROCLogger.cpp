...
static const char* mylogger_spec[] = {
    "implementation_id", "NIMEASIROCLogger",
    "type_name",         "NIMEASIROCLogger",
...
NIMEASIROCLogger::NIMEASIROCLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("nimeasiroclogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("nimeasiroclogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("NIMEASIROCLogger");
}

NIMEASIROCLogger::~NIMEASIROCLogger()
{
}

RTC::ReturnCode_t NIMEASIROCLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIMEASIROCLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIMEASIROCLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIMEASIROCLogger::daq_dummy()
{
    return 0;
}

int NIMEASIROCLogger::daq_configure()
{
    std::cerr << "*** NIMEASIROCLogger::configure" << std::endl;
    int ret = 0;
    m_isDataLogging = false;
...
int NIMEASIROCLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "NIMEASIROCLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "NIMEASIROCLogger: Save to file: false\n";
            }
        }
...
int NIMEASIROCLogger::daq_unconfigure()
{
    std::cerr << "*** NIMEASIROCLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int NIMEASIROCLogger::daq_start()
{
    std::cerr << "*** NIMEASIROCLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: NIMEASIROCLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** NIMEASIROCLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int NIMEASIROCLogger::daq_stop()
{
    std::cerr << "*** NIMEASIROCLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "NIMEASIROCLogger::stop: close files \n";
        }
...
int NIMEASIROCLogger::daq_pause()
{
    std::cerr << "*** NIMEASIROCLogger::pause" << std::endl;
    return 0;
}

int NIMEASIROCLogger::daq_resume()
{
    std::cerr << "*** NIMEASIROCLogger::resume" << std::endl;
    return 0;
}

int NIMEASIROCLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** NIMEASIROCLogger::InPort flushed\n";
    }
    return 0;
}

void NIMEASIROCLogger::toLower(std::basic_string<char>& s)
{
...
int NIMEASIROCLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### NIMEASIROCLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "NIMEASIROCLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void NIMEASIROCLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<NIMEASIROCLogger>,
                                 RTC::Delete<NIMEASIROCLogger>);
    }
};
